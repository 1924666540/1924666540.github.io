...
connection.listen({
  ...
  onTextMessage: function (message) {
    ...
    if (currentLi.className != 'clearfix active') {
      ...
      //显示未读消息
      document.getElementsByClassName('one-text')[1].innerHTML = message.data;
    } else {
      //生成别人的气泡
...
function addFriendListEvent() {
  ...
  for (var i = 0; i < lis.length; i++) {
    lis[i].onclick = function () {
      ...
      document.getElementsByClassName('title')[0].textContent = str;
...
window.onload = function () {
  ...
  getId('send').onclick = function () {
    ...
    msg.set({
      ...
      //显示未读消息
      document.getElementsByClassName('one-text')[1].innerHTML = sendMsg;
      },
      error: function () {
        console.log('发送消息失败');
      }
  ...
  //回车发送
  getId('input-content').oninput = function () {
